# audio_mastering_engine.py (v9.1)
#
# This version fixes a NameError by reordering the function definitions.
#
...
def apply_saturation(samples, amount):
    """Applies harmonic saturation to the audio samples."""
    if amount == 0: return samples
    gain = 1.0 + (amount / 100.0) * 4.0
    processed_samples = np.tanh(samples * gain) / gain
...
def apply_multiband_compressor(chunk, settings):
    """Splits a chunk into 3 bands, compresses each with custom settings."""
...
# --- COMMAND-LINE INTERFACE (for legacy use) ---
def main():
    print("This script is intended to be used with mastering_gui.py")

# mastering_gui.py (v2.4)
#
# A graphical user interface for the Python Audio Mastering tool.
# This version increases the manual scaling factor for better readability.
#
...
# Import the engine and its presets
try:
    import audio_mastering_engine as engine
    EQ_PRESETS = engine.EQ_PRESETS
except ImportError:
    messagebox.showerror("Error", "The 'audio_mastering_engine.py' file was not found. Make sure it's in the same directory.")
    exit()
...
class MasteringApp(ThemedTk):
    def __init__(self):
        ...
        scaled_height = int(800 * self.SCALING_FACTOR)
        ...
        file_frame.pack(fill="x", expand=True, pady=(0, int(15 * self.SCALING_FACTOR)))
        ...
        preset_frame.pack(fill="x", expand=True, pady=int(15 * self.SCALING_FACTOR))
        ...
        controls_frame.pack(fill="x", expand=True, pady=int(15 * self.SCALING_FACTOR))

        self.bass_boost = self.create_slider(controls_frame, "Bass (dB)", -6.0, 6.0, 0.0, 0)
        self.mid_cut = self.create_slider(controls_frame, "Mid Cut (dB)", 0.0, 6.0, 0.0, 1)
        self.presence_boost = self.create_slider(controls_frame, "Presence (dB)", -6.0, 6.0, 0.0, 2)
        self.treble_boost = self.create_slider(controls_frame, "Treble (dB)", -6.0, 6.0, 0.0, 3)
        self.width = self.create_slider(controls_frame, "Stereo Width", 0.0, 2.0, 1.0, 4)
        self.lufs = self.create_slider(controls_frame, "Target LUFS", -24.0, -6.0, -14.0, 5)
        self.use_multiband = tk.BooleanVar()
        ttk.Checkbutton(controls_frame, text="Use Multiband Compressor", variable=self.use_multiband).grid(row=6, column=0, columnspan=3, sticky="w", pady=10)

        # --- Process Section ---
        process_frame = ttk.Frame(main_frame)
        process_frame.pack(fill="x", expand=True, pady=int(15 * self.SCALING_FACTOR))
    ...
    def create_slider(self, parent, label, from_, to, default, row):
        ...
        label_val = ttk.Label(parent, text=f"{default:.2f}", width=6)
        label_val.grid(row=row, column=2, sticky="w")
        slider.configure(command=lambda v, lbl=label_val: lbl.config(text=f"{float(v):.2f}"))
        ...
        def update_labels():
             for child in self.winfo_children()[0].winfo_children()[2].winfo_children():
                if isinstance(child, ttk.Label) and child.grid_info()['column'] == 2:
                    ...
                    if row == 0: child.config(text=f"{self.bass_boost.get():.2f}")
                    if row == 1: child.config(text=f"{self.mid_cut.get():.2f}")
                    if row == 2: child.config(text=f"{self.presence_boost.get():.2f}")
                    if row == 3: child.config(text=f"{self.treble_boost.get():.2f}")
